...
- A new or changed module gets an example that can DEMONSTRATE it and a `npm run module:probe -- <id>` run that passes: the probe audits the example (a time-domain effect needs a gated source and both output channels) and sweeps every knob, failing one that does not change the sound. Aggregate measurements — level, balance, correlation — do not catch a knob that does nothing; this does. See docs/adrs/0010.
...
## Rules — engine (C++)
- Audio thread code (`Module::process`, scheduler, param drain, program swap): no allocation, locks, syscalls, exceptions, logging. Tests assert this. `Engine::renderInterleaved` puts the thread in flush-to-zero first (`rt/Denormals.hpp`); denormals are a hundred-cycle slow path exactly where audio decays to silence.
