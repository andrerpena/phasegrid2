...
- Colours exist twice, in `css/theme.css` and `theming/themes/*.ts`, because CSS needs them for Tailwind and the Pixi canvas cannot read a stylesheet. `theming/theme-parity.test.ts` fails if the two disagree — change one and it names the other. Grid colours must be `#rrggbb`; `hexToNumber` parses nothing else.
