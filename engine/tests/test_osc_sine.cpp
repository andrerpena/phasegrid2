...
TEST_CASE("at Fold 0 the sine is the one wave with no harmonics", "[osc][sine]") {
...
TEST_CASE("the fold knob is what adds them", "[osc][sine]") {
  ...
  REQUIRE(contentOf(0.f).odd < 0.01);
...
TEST_CASE("fold turns one arch into many lobes", "[osc][sine]") {
  ...
  const auto turns = [](const std::vector<float>& wave) {
    ...
    for (size_t i = 1; i + 1 < wave.size(); ++i)
      if ((wave[i] - wave[i - 1]) * (wave[i + 1] - wave[i]) < 0.f) ++n;
    return n;
  };
...
TEST_CASE("the folded sine is band-limited", "[osc][sine]") {
  /*
   * Folding bends a wave rather than breaking it, so there is no step for the sawtooth's correction to
   * fix. What corners need instead is the average of the wave across each sample, which is what
   * `SineShape::at(from, to)` computes exactly, and it is worth about ten decibels: at 2 kHz, 21.6 dB
   * reading the wave at a point against 33.6 dB averaging it.
   *
   * What averaging cannot do is repeal the arithmetic. Driving the wave sixteen times over puts thirty
   * turns in every cycle, and at a high pitch their harmonics are simply past Nyquist. Measured, the
   * off-harmonic energy below the harmonics, in decibels:
   *
   *          0 st   12 st   24 st   36 st   48 st
   *   220 Hz  99.3    63.1    55.4    45.5    36.2
   *   439 Hz  99.1    53.8    44.7    35.4    26.9
   *   879 Hz  99.2    46.7    36.7    27.7    24.4
   *  2001 Hz  96.2    35.1    27.1    12.5     6.5
   *
   * So the bar sits where the instrument is played rather than where the arithmetic runs out: every
   * fold setting at A4, and a plain sine at any pitch at all. Fully folded above a kilohertz is outside
   * it, and the way in would be to oversample this one stage.
   */
  ...
  // Unfolded it is the one wave with nothing to alias, however high it is played.
